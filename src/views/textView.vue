<template>
  <div class="AllPosts">
    <h1>all text based posts page</h1>
...
export default {
  ...
  computed: {
    Posts() {
      return this.$store.state.alltText;
    },
    user() {
      ...
      fetch(
        ...
        .then((returnData) => {
          if (returnData.error) {
            ...
            let alltText = (this.$store.state.alltText = []);
            let i;
            let config0 = "!image";
            let config1 = "!video";
            let config2 = "!audio";
            for (i = 0; i < returnData.length; i++) {
              if (!returnData[i].message.startsWith(config0) && !returnData[i].message.startsWith(config1) && !returnData[i].message.startsWith(config2)) {
                alltText.push(returnData[i]);
              }
            }
          }
        });
